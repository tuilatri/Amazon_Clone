...
const CartReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case REMOVE_FROM_CART:
      return {
        ...state,
        items: state.items.filter(item => (item.product_id || item.id) !== action.payload)
      };
    //Update cart number
    case UPDATE_CART_QUANTITY:
      return {
        ...
        items: state.items.map(item =>
          (item.product_id || item.id) === action.payload.itemId ? { ...item, quantity: action.payload.quantity } : item
        )
      };
